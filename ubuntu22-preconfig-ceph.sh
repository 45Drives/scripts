...
|_____  ##|_____  ##| ##  | ##| ##  \__/| ## \  ##/##/| ########|  ###### 
      ...
      |__/ \______/ |_______/ |__/      |__/    \_/    \_______/|_______/
                                           ...
                                           Ubuntu22 Preconfiguration Script.
...
update_system() {
	...
	if [[ "$response" == "17" ]]; then
    	...
    	apt-add-repository 'deb https://download.ceph.com/debian-reef/ jammy main'
	elif [[ "$response" == "19" ]]; then
    	apt-add-repository 'deb https://download.ceph.com/debian-squid/ jammy main'	
	else
...
add_cockpit() {
	...
	#cockpit-benchmark cockpit-navigator cockpit-file-sharing cockpit-45drives-hardware cockpit-identities cockpit-sosreport realmd tuned udisks2-lvm2 samba winbind nfs-kernel-server nfs-client 45drives-tools cockpit-scheduler chrony
